...
router.post('/join', isNotLoggedIn, async (req, res) => {
    ...
    if (!licenseExp.test(bizz_license) && user_roll === 'bizz') {
        console.log(licenseExp.test(bizz_license));
        return res.send({status: 'error', message: '잘못된 사업자등록증 번호입니다'});
    } else {
        if (exUser) {
            ...
            try {
                ...
                await User.create({
                    user_id,
                    user_pwd: hash,
                    user_email, user_nickname, user_phone, user_roll,
